...
class AppDelegate: NSObject, NSApplicationDelegate, NSUserNotificationCenterDelegate {
    ...
    private var skipService: SkipServiceController?
    ...
    func applicationDidFinishLaunching(_ aNotification: Notification) {
        ...
        skipService = SkipServiceController()
        skipService?.setup()
        let racesPanel = DisabledRacesPanelController()
        racesPanel.onSkip = { [weak self] in
            self?.skipService?.skipNow()
        }
        skipService?.onFeedback = { [weak racesPanel] text in
    ...
    func buildMenu() {
        DispatchQueue.main.async { [weak self] in
            ...
            skipService?.installDockMenu(dockMenu)
        }
    }
